...
const Searchbar: FC<Props> = () => {
 ...
 isOpen={isOpen}>
  <SearchModalContent initialSearch={q && String(q)} onSearch={(term: string) => {
    ...
    <Themed.div ref={buttonRef} as={Button} mx={2} onClick={() => setIsOpen(!isOpen)} aria-label="Search">
      { isOpen ? <Cross /> : (        <svg       width="20"
      height="22"
      viewBox="0 0 20 22"
      fill="none"
      stroke="currentColor"
...
const SearchModalContent = (props: { initialSearch?: string, onSearch: (term: string) => any }) => {
    ...
    return <Themed.div sx={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', p: [1,2] , width: '100%'}} >
      <Input type="search" sx={{ marginBottom: 15 }} defaultValue={props.initialSearch} placeholder="Search for products..." onChange={(event) => throttleSearch(event.target.value)} />
      {  loading ? (
              <LoadingDots />
            ) : products.length ? (
              <ProductGrid cardProps={{
                imgHeight: 540,
                imgWidth: 540,
                imgPriority: false,
              }} products={products} offset={0} limit={products.length}></ProductGrid>
             ) : (
              <span
              >
                {search ? (
